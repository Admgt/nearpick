# Dokumentációs index

Ez a mappa tartalmazza a NearPick projekt termékminőségi dokumentációs csomagját.
A projekt indításához és futtatási parancsaihoz használd a [`../README.md`](../README.md) fájlt.

## Termék

- Vision: [`01_product/vision.md`](01_product/vision.md)
- Scope contract: [`01_product/scope_contract.md`](01_product/scope_contract.md)
- Képességtérkép: [`01_product/capability_map.md`](01_product/capability_map.md)
- Metrikák: [`01_product/metrics.md`](01_product/metrics.md)

## Architektúra

- C4 kontextus és konténerek: [`02_architecture/c4_context_container.md`](02_architecture/c4_context_container.md)
- C4 komponensnézet: [`02_architecture/c4_component.md`](02_architecture/c4_component.md)
- Minőségi attribútumok: [`02_architecture/quality_attributes.md`](02_architecture/quality_attributes.md)
- Meglévő ADR hivatkozások:
  - [`../sprints/01/architecture/adr/0001-first-tech-choice.md`](../sprints/01/architecture/adr/0001-first-tech-choice.md)
  - [`../sprints/02/docs/adr/0001-deployment-target.md`](../sprints/02/docs/adr/0001-deployment-target.md)
  - [`../sprints/02/docs/adr/0002-platform-choice.md`](../sprints/02/docs/adr/0002-platform-choice.md)
  - [`../sprints/02/docs/adr/0003-iac-deploy-strategy.md`](../sprints/02/docs/adr/0003-iac-deploy-strategy.md)

## Tervezés

- API és szerződések: [`03_design/api.md`](03_design/api.md)
- Adatmodell: [`03_design/data_model.md`](03_design/data_model.md)
- Hibakezelés: [`03_design/error_handling.md`](03_design/error_handling.md)

## Minőség

- Tesztstratégia: [`04_quality/test_strategy.md`](04_quality/test_strategy.md)
- Tesztriport: [`04_quality/test_report.md`](04_quality/test_report.md)
- Test backlog: [`04_quality/test_backlog.md`](04_quality/test_backlog.md)

## Biztonság és üzemeltetés

- Fenyegetésmodell: [`05_security_ops/threat_model.md`](05_security_ops/threat_model.md)
- Adatvédelem és licencek: [`05_security_ops/privacy_licensing.md`](05_security_ops/privacy_licensing.md)
- Telepítési runbook: [`05_security_ops/deploy_runbook.md`](05_security_ops/deploy_runbook.md)
- Megfigyelhetőség: [`05_security_ops/observability.md`](05_security_ops/observability.md)

## Release

- Demóscript: [`06_release/demo_script.md`](06_release/demo_script.md)

## AI engineering

- AI manifest: [`07_ai/ai_manifest.md`](07_ai/ai_manifest.md)
- Promptnapló: [`07_ai/prompt_log.md`](07_ai/prompt_log.md)
- Verifikációs napló: [`07_ai/verification_log.md`](07_ai/verification_log.md)

## Állapot

Ez a struktúra végponttól végpontig létre van hozva. Egyes részek teljesek, mások részlegesek, és a fájlok explicit módon jelölik az ismert hiányokat.
