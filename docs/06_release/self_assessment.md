...
- A pontozás a jelenlegi kód + dokumentáció auditjára támaszkodik; friss lokális Flutter / Functions runtime tesztevidence, 2026-04-11-es Android emulatoros `integration_test` evidence és 2026-04-12-i célzott admin moderation `integration_test` evidence is rögzítve van. A CI evidence-t minden release-közeli push után külön frissíteni kell.
...
| Minimum tesztmix: 30+ automata teszt | Igen | [`test_strategy.md`](../04_quality/test_strategy.md), [`test_report.md`](../04_quality/test_report.md) | A statikus inventory 167 definíciót jelez, a friss runtime evidence pedig Flutter + Functions futást és három `integration_test` flow-t is alátámaszt |
